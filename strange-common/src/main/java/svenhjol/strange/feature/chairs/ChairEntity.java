    ...
    public void tick() {
        ...
        if (!level.isClientSide()) {
            ...
            if (!(block instanceof StairBlock)) {
                ...
                Strange.LOG.debug(getClass(), "Removing because no longer a stairs block");
            } else if (stateAbove.isCollisionShapeFullBlock(level, pos.above())) {
                unRide();
                remove(RemovalReason.DISCARDED);
                Strange.LOG.debug(getClass(), "Removing because block above is invalid");
            } else if (!hasExactlyOnePlayerPassenger()) {
                remove(RemovalReason.DISCARDED);
                Strange.LOG.debug(getClass(), "Removing because no passengers");
            }
        }
    }
