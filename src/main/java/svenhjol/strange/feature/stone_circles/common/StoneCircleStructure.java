...
public class StoneCircleStructure extends Structure implements FeatureResolver<StoneCircles> {
    ...
    public static final MapCodec<StoneCircleStructure> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
        ...
        STONE_CIRCLES.providers.codec.fieldOf("stone_circle_definition")
    ...
    protected StoneCircleStructure(StructureSettings settings, StoneCircleDefinition definition) {
        ...
        var defaultDefinition = STONE_CIRCLES.providers.definitions.get(Providers.DEFAULT);
        if (defaultDefinition == null) {
            throw new RuntimeException("Missing default stone circle definition");
        }

        this.definition = STONE_CIRCLES.providers.definitions.getOrDefault(definition.getSerializedName(), defaultDefinition);
    }
