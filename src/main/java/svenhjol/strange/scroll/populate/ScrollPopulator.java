...
public abstract class ScrollPopulator {
    ...
    public ScrollPopulator(World world, BlockPos pos, Scroll scroll, ScrollDefinition definition) {
        this.world = world;
        this.pos = pos;
        this.scroll = scroll;
    ...
    public String filterRarity(String key) {
        if (key.contains(EPIC_LABEL)) {
            ...
            if (world.random.nextFloat() > (EPIC_CHANCE_BASE + (0.02F * scroll.getRarity()))) return null;
            ...
            if (world.random.nextFloat() > (RARE_CHANCE_BASE + (0.02F * scroll.getRarity()))) return null;
            return key.replace(RARE_LABEL, "");

        } else if (key.contains(UNCOMMON_LABEL)) {
            // uncommon items pass sometimes
            if (world.random.nextFloat() > (UNCOMMON_CHANCE_BASE + (0.05F * scroll.getRarity()))) return null;
            return key.replace(UNCOMMON_LABEL, "");

        } else if (key.contains(COMMON_LABEL)) {
            // common items pass often
            if (world.random.nextFloat() > (COMMON_CHANCE_BASE + (0.05F * scroll.getRarity()))) return null;
    ...
    public int multiplyValue(int original) {
        return original * scroll.getRarity();
    }
}
