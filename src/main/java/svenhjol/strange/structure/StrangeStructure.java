...
public abstract class StrangeStructure {
    ...
    protected void addStart(String pieceName, int weight, StructureProcessorList processors) {
        String piecePath = getPiecePath(pieceName);
        starts.add(Pair.of(StructurePoolElement.single(piecePath, processors), weight));
    }
    ...
    protected void registerPool(String poolName, Map<String, Integer> elements, StructureTemplatePool.Projection projection, StructureProcessorList processors) {
        ...
        ResourceLocation poolNamePath = getPoolPath(poolName);
        ResourceLocation poolEndsPath = getPoolPath("ends");
